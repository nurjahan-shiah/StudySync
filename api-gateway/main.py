...
def validate_access_token(
    ...
    """
    Validate the Bearer access token for a protected route.
    Returns the authenticated user's ID, email, and role.
    Raises HTTPException if the header or token is invalid.
    """
...
def get_service_for_route(path: str) -> str:
    ...
    # Session routes live there, not in groups-service.
    if re.match(
        r"^/groups/[^/]+/sessions(/.*)?$",
        path
    ):
...
async def forward_request(
    ...
    # Add trusted identity headers after validating the JWT
...
async def gateway(
    ...
    if full_path not in PUBLIC_ROUTES:
        # Most routes require auth - validate the Bearer token
